...
function CheckOutOrder() {
	...
	const { data, isSuccess, isError, isLoading, error } = useGetSingleOrderQuery(id);
						...
						<div className="mt-5 md:mt-0 md:col-span-2">
							<Formik
								initialValues={{
									...
									lastName: '',
									email: order?.emaill || user?.user?.email,
									phone: order?.phone || user?.user?.phone,
									address: order?.address || user?.user?.address,
									city: order?.city || user?.user?.city,
									country: order?.country || user?.user?.country,
									zipCode: order?.zipCode || user?.user?.zipCode,
